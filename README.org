...
If you plan on using more than one of these with ~self-insert-command~, you'll need to either use =:lambda t= or the =:name= keyword argument to prevent the newly created functions from clobbering each other.
