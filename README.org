...
~general-add-hook~ can optionally remove the function from the hook after it runs once or after it returns non-nil. For an example of this, see ~general-after-gui~. If you would like this functionality for ~general-add-advice~, feel free to make a feature request.

** Miscellaneous
~general-after-init~ can be used to run code after initialization (e.g. ~(general-after-init (do-something) (do-something-else))~). It just adds to =after-init-hook=.
