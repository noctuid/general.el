...
For now there is no ~general-setq-default~ or ~general-setq-local~. The reason for this is that I have never seen any setters that handle both local and default variable changes (they generally just use ~set-default~). If you are aware of a situation where it might be useful to have these setters, please make an issue.
