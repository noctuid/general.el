...
When you're defining keys in specific keymaps and states, using positional arguments can be shorter. General has two functions that have the equivalent positional arguments of ~define-key~ and ~evil-define-key~. They are ~general-emacs-define-key~ and ~general-evil-define-key~ respectively. These are simply wrappers for ~general-define-key~ that pass the positional arguments to the corresponding keywords.
...
The actual behavior of these two functions is the same as ~general-define-key~. You can still use ~general-define-key~'s keyword arguments after the positional arguments:
#+begin_src emacs-lisp
