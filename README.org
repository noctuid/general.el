...
Besides the state keymaps, you can also use =inner= and =outer= in place of =evil-inner-text-objects-map= and =evil-outer-text-objects-map=. All states have shorter one-letter abbreviations as well (or =in= and =out= in the case of the text object keymaps):
#+begin_src emacs-lisp
(general-define-key :keymaps 'v
                    ...)

(general-define-key :keymaps '(in out)
                    ...)
#+end_src
